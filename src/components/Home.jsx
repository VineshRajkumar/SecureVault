...
const Home = () => {
  ...
  useEffect(() => {
    //previous before storing the data in data key i stored it in password key so if you check that it will contain previous data stored
    let pass = localStorage.getItem("data"); //data here is a key where it contains all the data (you can give any key name) this is returned in form of string
    if (pass) {
      setFormArray(JSON.parse(pass)); //JSON.parse converts this string to array and JSON.stringify converts array to string
    }
  }, []);

  const SavePasswordOnClick = () => {
    if (
      form.url.length >= 4 &&
  ...
  const handleEditToLocalStrg = (index) => {
  ...
  const handleDeleteFromLocalStrg = (index) => {
